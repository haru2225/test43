...
test43 started as a narrowed ablation of test40 (same architecture, phase
...
def load_dataset(path):
    ...
    if meta["format"] not in (FORMAT, "test43-crystal-single-phase-v1",
                              "test42-crystal-single-phase-v1", "test42-crystal-single-phase-v2"):
        raise ValueError("Not a test43 dataset")
...
def generate(args):
    ...
    with (out / "final.data").open("w") as stream:
        stream.write("test43 SiO4 CG positions; structure generator, no force field supplied\n\n")
...
def evaluate(args):
    ...
    axes[1].legend()
    fig.suptitle("test43 SiO4 beads: crystal (single-phase ablation)")
    fig.tight_layout()
